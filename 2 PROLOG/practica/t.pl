...
bienUbicado(Persona) :-
    not((
        quiereIr(Persona, Destino),
        not(viveEnZona(Persona, Destino))
    )).

bienUbicadov1(Persona) :-
        not(quiereIr(Persona, Destino)),
        not(viveEnZona(Persona, Destino)).

viveEnZona(Persona, Lugar) :-
	quedaEn(Lugar, Ubicacion),
